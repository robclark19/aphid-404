...
rmd_split <- createDataPartition(model_data$miridae, p = 0.7, list = FALSE)
rmd_train <- model_data[md_split, !(names(model_data) %in% c('date', 'site', 'transect', 'sampling_period'))]
rmd_train_sub <- rmd_train %>% dplyr::select(which(!colSums(rmd_train, na.rm=TRUE) %in% 0)) 
rmd_test <- model_data[-md_split, !(names(model_data) %in% c('date', 'site', 'transect', 'sampling_period'))]
...
ml_formula <- as.formula(paste0(insects_interest[2],
                                "~", "."))
#Data split
cmd_split <- createDataPartition(model_data$miridae, p = 0.7, list = FALSE)
cmd_train <- model_data[md_split,!(names(model_data) %in% c('date', 'site', 'transect', 'sampling_period'))]
cmd_train_sub <- cmd_train %>% dplyr::select(which(!colSums(cmd_train, na.rm=TRUE) %in% 0)) 
cmd_test <- model_data[-md_split,!(names(model_data) %in% c('date', 'site', 'transect', 'sampling_period'))]
...
##### Ridge regression
set.seed(7)
ridge_cmod <- train(form = ml_formula, data = cmd_train, method = 'glmnet', tuneGrid = expand.grid(alpha = 0, lambda = 1))
